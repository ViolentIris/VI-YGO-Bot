...
namespace WindBot.Game.AI.Decks
{
    ...
    class TearlamentsExecutor : DefaultExecutor
    {
        ...
        List<int> no_link_ids = new List<int>() { CardId.TearlamentsRulkallos, CardId.BaronnedeFleur, CardId.MekkKnightCrusadiaAvramax, CardId.AbyssDweller, CardId.PredaplantDragostapelia, CardId.UnderworldGoddessoftheClosedWorld, CardId.EvilswarmExcitonKnight, CardId.TearlamentsKaleidoHeart };
  ...
  : base(ai, duel)
        ...
        public void OnSelectChain(IList<ClientCard> cards)
        {
        ...
        public override IList<ClientCard> OnSelectCard(IList<ClientCard> cards, int min, int max, int hint, bool cancelable)
        {
            ...
            if (hint == HintMsg.SpSummon && cards.Any(card => card != null && (card.Level == 2 || card.LinkCount == 2) && card.Location == CardLocation.Grave))
            {
                ...
                else
                {
                    ...
                    else if (HasInList(cards, CardId.DivineroftheHerald) && GetZoneCards(CardLocation.Onfield, Enemy).Any(card => card != null && !card.IsShouldNotBeTarget()))
                    {
                        ids.Add(CardId.DivineroftheHerald);
                    }
        ...
        private bool KnightmareUnicornEffect()
        {
            ...
            if (!activate_KelbektheAncientVanguard_2) ids.Add(CardId.KelbektheAncientVanguard);
            if (!activate_AgidotheAncientSentinel_2) ids.Add(CardId.AgidotheAncientSentinel);
            ids.AddRange(new List<int>() { CardId.MudoratheSwordOracle, CardId.KeldotheSacredProtector, CardId.ShaddollBeast, CardId.ShaddollDragon, CardId.TearlamentsSulliek, CardId.TearlamentsReinoheart });
            cards.Sort(CardContainer.CompareCardAttack);
            cards.Reverse();
            AI.SelectCard(ids);
